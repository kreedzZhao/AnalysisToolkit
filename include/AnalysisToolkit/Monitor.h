...
namespace AnalysisToolkit {
    ...
    // Monitor 辅助宏
    #define ATKIT_JNI_MONITOR_INIT(env, config) \
        Monitor::getInstance()->enableJNIMonitoring(env, config)

    #define ATKIT_JNI_MONITOR_START() \
        Monitor::getInstance()->getJNIMonitor()->startMonitoring()

    #define ATKIT_JNI_MONITOR_STOP() \
        Monitor::getInstance()->getJNIMonitor()->stopMonitoring()
