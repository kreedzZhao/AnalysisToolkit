...
namespace AnalysisToolkit {
    ...
    JNI_HOOK_DEF(jobject, CallObjectMethodV, JNIEnv* env, jobject obj, jmethodID method_id, va_list args) {
        GET_JNI_MONITOR()
    ...
    bool JNIMonitor::initializeJNIHooks() {
        ...
        if (config_.enable_method_calls) {
            ...
            auto status3 = hook_manager->hookFunction(
                reinterpret_cast<void*>(jni_env_->functions->CallIntMethodV),
            ...
            all_success = all_success && 
                         (status1 == HookStatus::SUCCESS) && 
                         (status2 == HookStatus::SUCCESS) &&
                         (status3 == HookStatus::SUCCESS);
        }

        // Hook 字符串操作
        if (config_.enable_string_operations) {
            auto status1 = hook_manager->hookFunction(
                reinterpret_cast<void*>(jni_env_->functions->NewStringUTF),
        ...
        // Hook 类查找
        auto status = hook_manager->hookFunction(
            reinterpret_cast<void*>(jni_env_->functions->FindClass),
    ...
    void JNIMonitor::setAuxiliaryClass(jclass clazz) {
        ...
        if (jni_env_ && clazz) {
            to_string_method_ = jni_env_->GetStaticMethodID(
                clazz, "toString", "(Ljava/lang/Object;)Ljava/lang/String;"
            );
        }
    ...
    namespace MonitorUtils {
        ...
        std::string jvalueToString(JNIEnv* env, jvalue value, char type) {
            switch (type) {
                case 'Z': return value.z ? "true" : "false";
                case 'B': return std::to_string(value.b);
                case 'C': return std::string(1, static_cast<char>(value.c));
                case 'S': return std::to_string(value.s);
                case 'I': return std::to_string(value.i);
                case 'J': return std::to_string(value.j);
                case 'F': return std::to_string(value.f);
                case 'D': return std::to_string(value.d);
                case 'L': {
                    if (value.l && env) {
        ...
        bool isSystemClass(const std::string& class_name) {
            return class_name.find("java.") == 0 || 
                   class_name.find("android.") == 0 ||
