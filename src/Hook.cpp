...
// 引入 dobby header，为了避免依赖问题，我们使用函数指针声明
extern "C" {
...
namespace AnalysisToolkit {
    ...
    std::unique_ptr<HookManager> HookManager::instance_ = nullptr;
    std::mutex HookManager::instance_mutex_;

    HookManager* HookManager::getInstance() {
        std::lock_guard<std::mutex> lock(instance_mutex_);
        if (instance_ == nullptr) {
            instance_ = std::unique_ptr<HookManager>(new HookManager());
        }
    ...
    void* HookManager::resolveSymbol(const std::string& library_name, const std::string& symbol_name) {
        ...
        if (symbol == nullptr) {
            ATKIT_ERROR("Failed to find symbol %s in %s: %s", 
                       symbol_name.c_str(), library_name.c_str(), dlerror());
        }
    ...
    HookStatus HookManager::hookFunction(void* target_address, 
        ...
        if (it != active_hooks_.end() && it->second->is_active) {
            ATKIT_WARN("Address %p already hooked with tag: %s", 
                      target_address, it->second->symbol_name.c_str());
    ...
    HookStatus HookManager::hookSymbol(const std::string& library_name,
        ...
        if (target_address == nullptr) {
            ATKIT_ERROR("Failed to resolve symbol %s in library %s", 
                       symbol_name.c_str(), library_name.c_str());
    ...
    HookStatus HookManager::instrumentFunction(void* target_address,
        ...
        auto* wrapper_callback = new std::function<void(void*, DobbyRegisterContext*)>(
            [pre_callback](void* address, DobbyRegisterContext* ctx) {
                pre_callback(address, ctx);
            }
        );

        dobby_instrument_callback_t dobby_callback = 
        ...
        if (result != 0) {
            ...
            ATKIT_ERROR("Dobby instrument failed for address %p, error code: %d", target_address, result);
            return HookStatus::FAILED;
        }
        ...
        ATKIT_INFO("Successfully instrumented function at %p with tag: %s", target_address, tag.c_str());
        return HookStatus::SUCCESS;
    }
    ...
    void* HookManager::getSymbolAddress(const std::string& library_name, const std::string& symbol_name) {
