...
namespace AnalysisToolkit {
    ...
    class HookManager {
    private:
        static std::unique_ptr<HookManager> instance_;
    ...
    #define ATKIT_HOOK_SYMBOL(lib_name, symbol, func_name) \
        HookManager::getInstance()->hookSymbol(lib_name, symbol, \
            reinterpret_cast<void*>(hooked_##func_name), \
            reinterpret_cast<void**>(&original_##func_name), #func_name)
    ...
    #define ATKIT_UNHOOK(address) \
        HookManager::getInstance()->unhookFunction(address)
