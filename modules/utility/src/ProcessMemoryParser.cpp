...
MemoryRegion::MemoryRegion(uintptr_t start,
                           uintptr_t end,
                           const MemoryPermissions& perms,
                           uint64_t offset,
                           const std::string& device,
                           uint32_t inode,
                           const std::string& pathname,
                           const std::string& original_line)
    : start_address_(start),
      end_address_(end),
      permissions_(perms),
      offset_(offset),
      device_(device),
...
std::string MemoryRegion::toString() const {
    ...
    oss << "0x" << std::hex << start_address_ << "-0x" << end_address_ << " "
        << permissions_.toString() << " 0x" << std::setfill('0') << std::setw(8) << offset_ << " "
        << device_ << " " << std::dec << inode_ << " "
...
ProcessMemoryParser::Result<std::vector<MemoryRegion>> ProcessMemoryParser::findRegionsContaining(
    uintptr_t address,
    int pid) {
...
ProcessMemoryParser::findRegionsByPath(const std::string& pathname, int pid, bool exact_match) {
    ...
    std::vector<MemoryRegion> matching_regions;
    for (const auto& region : parse_result.getValue()) {
        bool matches = exact_match ? (region.getPathname() == pathname)
...
ProcessMemoryParser::findRegionsByPermissions(const MemoryPermissions& permissions, int pid) {
    ...
    for (const auto& region : parse_result.getValue()) {
        ...
        if (permissions.readable && !region_perms.readable)
            matches = false;
        if (permissions.writable && !region_perms.writable)
            matches = false;
        if (permissions.executable && !region_perms.executable)
            matches = false;
        if (permissions.private_mapping && !region_perms.private_mapping)
            matches = false;
...
void ProcessMemoryParser::printMemoryMap(const std::vector<MemoryRegion>& regions, int limit) {
    std::cout << std::left << std::setw(20) << "Address Range" << std::setw(8) << "Perms"
              << std::setw(12) << "Offset" << std::setw(12) << "Device" << std::setw(8) << "Inode"
              << std::setw(12) << "Size"
              << "Pathname" << std::endl;
    ...
    for (const auto& region : regions) {
        ...
        std::cout << std::hex << "0x" << std::setfill('0') << std::setw(8)
                  << region.getStartAddress() << "-0x" << std::setw(8) << region.getEndAddress()
                  << " " << std::setfill(' ') << std::setw(4) << region.getPermissions().toString()
                  << " " << std::hex << "0x" << std::setfill('0') << std::setw(8)
                  << region.getOffset() << " " << std::setfill(' ') << std::setw(8)
                  << region.getDevice() << " " << std::dec << std::setw(6) << region.getInode()
...
#ifdef __linux__
ProcessMemoryParser::Result<std::vector<MemoryRegion>> ProcessMemoryParser::parseLinuxMaps(
    int pid) {
...
#ifdef __APPLE__
ProcessMemoryParser::Result<std::vector<MemoryRegion>> ProcessMemoryParser::parseMacOSMaps(
    int pid) {
