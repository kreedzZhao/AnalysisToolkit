...
class MemoryRegion {
  public:
    ...
    size_t getSize() const {
        return end_address_ - start_address_;
    }
    const MemoryPermissions& getPermissions() const {
        return permissions_;
    }
    ...
    bool isAnonymous() const {
        return pathname_.empty() || pathname_ == "[anon]";
    }
    bool isStack() const {
        return pathname_ == "[stack]";
    }
    bool isHeap() const {
        return pathname_ == "[heap]";
    }
    bool isVdso() const {
        return pathname_ == "[vdso]";
    }
...
class ProcessMemoryParser {
  public:
     ...
     * @return Result containing vector of memory regions or error
     */
    Result<std::vector<MemoryRegion>> parseSelf() {
        return parseProcess(-1);
    }
