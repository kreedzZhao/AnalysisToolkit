...
void demonstrateAddressSearch() {
    ...
    std::cout << "Searching for regions containing address: 0x" 
              << std::hex << func_address << std::dec << std::endl;
...
void demonstrateCustomFiltering() {
    ...
    for (const auto& region : regions) {
        std::cout << "  " << region.toString() 
                  << " (Size: " << (region.getSize() / 1024 / 1024) << " MB)" << std::endl;
    }
...
int main() {
    ...
    std::cout << "Platform supported: " << (ProcessMemoryParser::isPlatformSupported() ? "Yes" : "No") << std::endl;
