...
TEST_F(ProcessMemoryParserTest, ErrorStringConversion) {
    ...
    error_str =
        ProcessMemoryParser::getErrorString(ProcessMemoryParser::ErrorCode::PERMISSION_DENIED);
    EXPECT_EQ(error_str, "Permission denied");

    error_str =
        ProcessMemoryParser::getErrorString(ProcessMemoryParser::ErrorCode::PLATFORM_NOT_SUPPORTED);
...
TEST_F(ProcessMemoryParserTest, CustomFiltering) {
    ...
    parser->setRegionFilter([](const MemoryRegion& region) { return region.getSize() >= 4096; });
...
TEST_F(ProcessMemoryParserTest, ResultClass) {
    ...
    auto success_result =
        ProcessMemoryParser::Result<std::vector<MemoryRegion>>(std::move(regions));
    ...
    auto error_result = ProcessMemoryParser::Result<std::vector<MemoryRegion>>(
        ProcessMemoryParser::ErrorCode::PERMISSION_DENIED,
        "Test error");
